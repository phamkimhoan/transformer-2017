...
from config import (
    NUMBER_OF_LAYERS, DIM_MODEL, DIM_FEEDFORWARD,
    NUMBER_OF_ATTENTION_HEAD, DROP_OUT, TRAIN_CONFIG,
)
from utils import (
...
def train_worker(
    gpu, ngpus_per_node, vocab_src, vocab_tgt,
    spacy_src, spacy_tgt, config, is_distributed=False,
):
    ...
    model = make_model(len(vocab_src), len(vocab_tgt),
                       N=NUMBER_OF_LAYERS, d_model=DIM_MODEL,
                       d_ff=DIM_FEEDFORWARD, h=NUMBER_OF_ATTENTION_HEAD, dropout=DROP_OUT)
    model.to(device)
    module = model
    ...
    ckpt_path = find_checkpoint(config.get('directory'), config['file_prefix'])
    if ckpt_path:
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        print(f'[{device}] train loss: {train_loss:.4f} | val loss: {val_loss:.4f}', flush=True)
        empty_cache(device_type)
...
def train_model(vocab_src, vocab_tgt, spacy_src, spacy_tgt, config):
    device_type = get_device()
    ...
    if config.get('distributed', False) and device_type == "cuda":
        ...
        mp.spawn(
            ...
            args=(ngpus, vocab_src, vocab_tgt, spacy_src, spacy_tgt, config, True),
        )
    else:
        train_worker(
            0, 1, vocab_src, vocab_tgt, spacy_src, spacy_tgt, config, False
        )


if __name__ == "__main__":
    spacy_src, spacy_tgt = load_tokenizers()
    vocab_src, vocab_tgt = load_vocab(spacy_src, spacy_tgt)
    train_model(vocab_src, vocab_tgt, spacy_src, spacy_tgt, TRAIN_CONFIG)